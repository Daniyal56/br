...
def app():
    ...
    if lov_selection_col == 'Comparison':
        ...
        st.write(' ')
        st.write(' ')
        bank_note = '<p style="font-family:Roboto;"><b>Please select your Bank file first.</b></p>'
        ...
        try:
            ...
            with bnk_chq_no:
                bnk_chq_no_col = st.text_input('Please type CHQ Column of {}'.format(files[1].name))
            with bnk_debit:
                bnk_debit_col = st.text_input('Please type Debit Column of {}'.format(files[1].name))
            with bnk_crdt:
                bnk_crdt_col = st.text_input('Please type Credit Column of {}'.format(files[1].name))
            # Company Columns
            cmp_chq_no, cmp_debit, cmp_crdt = st.beta_columns(3)
            with cmp_chq_no:
                cmp_chq_no_col = st.text_input('Please type CHQ Column of {}'.format(files[0].name))
            with cmp_debit:
                cmp_debit_col = st.text_input('Please type Debit Column of {}'.format(files[0].name))
            with cmp_crdt:
                cmp_crdt_col = st.text_input('Please type Credit Column of {}'.format(files[0].name))
            st.write('')
            #Bank difference
            ...
            indexes = bank[~(bank[bnk_chq_no_col].isin(company[cmp_chq_no_col])) | (
                bank[bnk_crdt_col].isin(company[cmp_debit_col])) | (bank[bnk_debit_col].isin(company[cmp_crdt_col]))]
            length = len(indexes)
            ...
            for idx, s in enumerate(indexes['status']):
                x = header[3].selectbox(f'', lov, key=f'_{idx}')
                status.append(x)
            indexes['status'] = status

            indexes.to_csv(f'Unmatched_{files[0].name}',index=False)
            sub_btn, v, t, y, u, i, o, e, f, sb_btn = st.beta_columns(10)
            with sb_btn:
                sb__btn = st.button("Convert-PDF")
            indexes = company[~(company[cmp_chq_no_col].isin(bank[bnk_chq_no_col])) | (
                company[cmp_debit_col].isin(bank[bnk_crdt_col])) | (company[cmp_crdt_col].isin(bank[bnk_debit_col]))]
            length = len(indexes)
            ...
            #Matched Records
            indexes = bank[(bank[bnk_chq_no_col].isin(company[cmp_chq_no_col])) | (bank[bnk_crdt_col].isin(company[cmp_debit_col])) | (bank[bnk_debit_col].isin(company[cmp_crdt_col]))]
            length = len(indexes)
            ...
            st.text('')
            st.text('')
            t = "<p style='text-align:center;'><span class='highlight red' style='text-align:center; font-family:Roboto; font-size:30.5px;'><b><u>Download Section</u>:</b> </span></p>"
            st.markdown(t, unsafe_allow_html=True)
            ...
            # st.warning("Please upload maximum 2 CSV files")
            pass
